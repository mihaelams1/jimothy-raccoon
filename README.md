...
Jimothy is an **original** character — a raccoon with short-spine syndrome (a
compact, stubby body) and a stub tail. The run-cycle sprite sheet is generated
procedurally from hand-authored parametric SVG in
[`tools/gen_jimothy.js`](tools/gen_jimothy.js); nothing is copied from any other
source. To regenerate the sheet after tweaking the art:

```
npm install        # one-time, pulls in `sharp`
node tools/gen_jimothy.js
```
...
- **Jimothy sprite** — original artwork, generated by `tools/gen_jimothy.js`.
